...
def compute_doa_scores_regr(pred_doa_rad, gt_doa_rad, pred_sed, gt_sed):
    ...
    for frame_cnt, sed_frame in enumerate(gt_sed):
        ...
        if nb_src_gt_list[frame_cnt] and nb_src_pred_list[frame_cnt]:
            ...
            doa_frame_gt_azi = gt_doa_rad[frame_cnt][sed_frame_gt == 1]

            sed_frame_pred = pred_sed[frame_cnt]
            doa_frame_pred_azi = pred_doa_rad[frame_cnt][sed_frame_pred == 1]

            doa_loss_pred += distance_between_gt_pred(doa_frame_gt_azi.T,
                                                      doa_frame_pred_azi.T)
...
def distance_between_gt_pred(gt_list_rad, pred_list_rad):
    ...
    if gt_len and pred_len:
        az1, az2= gt_list_rad[ind_pairs[:, 0]], pred_list_rad[ind_pairs[:, 1]]
        cost_mat[ind_pairs[:, 0], ind_pairs[:, 1]] = distance_between_spherical_coordinates_rad(az1, 0, az2, 0)
    ...
    for _frame_ind in range(_sed_labels.shape[0]):
        ...
        if len(_tmp_ind[0]):
            ...
            for _tmp_class in _tmp_ind[0]:
                _output_dict[_frame_ind].append([_tmp_class, _azi_labels[_frame_ind, _tmp_class]])
    return _output_dict
        ...
        if len(_tmp_class_ind[0]):
            ...
            for _tmp_class in _tmp_class_ind[0]:
                ...
                for _tmp_spatial in _tmp_spatial_ind[0]:
                    _azi = _feat_cls.get_matrix_index(_tmp_spatial)
...
def description_file_to_output_format(_desc_file_dict, _unique_classes, _hop_length_sec):
    ...
    for _ind, _tmp_start_sec in enumerate(_desc_file_dict['start']):
        ...
        _tmp_azi = _desc_file_dict['azi'][_ind]*180/np.pi
        _tmp_ele = _desc_file_dict['ele'][_ind]*180/np.pi
        ...
        for _frame_ind in range(_start_frame, _end_frame + 1):
            if _frame_ind not in _output_dict:
                _output_dict[_frame_ind] = []
            _output_dict[_frame_ind].append([_tmp_class, _tmp_azi])
...
def load_output_format_file(_output_format_file):
    ...
    for _line in _fid:
        ...
        if _frame_ind not in _output_dict:
            _output_dict[_frame_ind] = []
        _output_dict[_frame_ind].append([int(_words[1]), int(_words[2])])
    _fid.close()
    return _output_dict
...
def write_output_format_file(_output_format_file, _output_format_dict):
    ...
    for _frame_ind in _output_format_dict.keys():
        for _value in _output_format_dict[_frame_ind]:
            _fid.write('{},{},{}\n'.format(int(_frame_ind), int(_value[0]), int(_value[1])))
    _fid.close()
