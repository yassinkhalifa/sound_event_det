...
class DataGenerator(Sequence):
    def __init__(self, datadir = '', labelsdir = '', shuffle=False, perfile=False, train=True, default_azi=2*np.pi, default_ele=0, splits = [0], win_len = 480, hop_len = 240, parsing_len=96000, nfft = 2048, nb_channels = 4, nb_classes = 11, batch_size = 32):
        ...
        self._parsing_len = parsing_len
        self._nfft = nfft
        self._nb_channels = nb_channels
        self._nb_classes = nb_classes
        self._splits = splits
        self._batch_size = batch_size
        self._seq_len = int(self._parsing_len/self._hop_len)
    ...
    def __data_generation(self, index):
        ...
        batch_x = np.zeros((self._batch_size, 2*self._nb_channels, self._seq_len, int(self._nfft/2)))
        batch_y = np.zeros((self._batch_size, self._seq_len, 2*self._nb_classes))
        seq_count = 0
        for seq_cnt in range(index*self._batch_size, (index+1)*self._batch_size):
            ...
            batch_x[seq_count,:,:,:] = np.transpose(data_x[batch_info['start_idx']:batch_info['end_idx'], :, :], (2, 0, 1))
            batch_y[seq_count,:,:] = data_y[batch_info['start_idx']:batch_info['end_idx'], :]
            seq_count += 1
        batch_y = [
            batch_y[:, :, :self._nb_classes],  # SED labels
            batch_y[:, :, self._nb_classes:2 * self._nb_classes]
            ]
    ...
    def get_val_labels(self):
        batch_y = np.zeros((self._nb_total_batches*self._batch_size, self._seq_len, 2*self._nb_classes))
        seq_count = 0
        ...
        batch_y = [
            batch_y[:, :, :self._nb_classes],  # SED labels
            batch_y[:, :, self._nb_classes:2 * self._nb_classes]  # DOA labels in radians
            ]
