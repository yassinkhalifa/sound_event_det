    ...
    for _key in _in_dict.keys():
        for _seld in _in_dict[_key]:
            if _seld[0] not in _out_dict:
                _out_dict[_seld[0]] = []
            _out_dict[_seld[0]].append([_key, _seld[0], _seld[1]])
    return _out_dict
...
#pred = '/home/yassin/Desktop/DCASE19/split1_ir4_ov1_88.csv'
pred = '/home/yassin/Desktop/DCASE19/seld_simulated/results/foa_dev_split_3/split3_simulated_IR_96.csv'
...
ref_dir = '/home/yassin/Desktop/DCASE19/seld_simulated/orig_labels/'
...
feat_cls = DataPreparation(datadir = '', labelsdir ='', outputdir = '')
...
ax3 = plot.subplot(gs[1, :2]), plot_func(ref_data, hop_s, ind=2), plot.ylim([-190, 190]), plot.title('Azimuth DOA reference')
ax4 = plot.subplot(gs[1, 2:]), plot_func(pred_data, hop_s, ind=2), plot.ylim([-190, 190]), plot.title('Azimuth DOA predicted')
