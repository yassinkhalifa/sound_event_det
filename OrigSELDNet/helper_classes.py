...
class DataGenerator(Sequence):
    ...
    def __data_generation(self, index):
        ...
        for seq_cnt in range(index*self._batch_size, (index+1)*self._batch_size):
            ...
            batch_x[seq_count,:,:,:] = np.transpose(data_x[batch_info['start_idx']:batch_info['end_idx'], :].reshape((self._seq_len, int(self._nfft/2), 2*self._nb_channels)), (2, 0, 1))
            batch_y[seq_count,:,:] = data_y[batch_info['start_idx']:batch_info['end_idx'], :]
            seq_count += 1
        azi_rad = batch_y[:, :, self._nb_classes:2 * self._nb_classes].reshape((self._batch_size, self._seq_len, self._nb_classes, 1)) * np.pi / 180
        ele_rad = batch_y[:, :, 2 * self._nb_classes:].reshape((self._batch_size, self._seq_len, self._nb_classes, 1)) * np.pi / self._default_ele
        azi_btrn = np.concatenate((np.cos(azi_rad), np.sin(azi_rad)), axis=-1)
        ele_btrn = np.concatenate((np.cos(ele_rad), np.sin(ele_rad)), axis=-1)
        batch_y = [
            batch_y[:, :, :self._nb_classes],  # SED labels
            np.concatenate((azi_btrn, ele_btrn), -2)  # DOA labels in radians
            ]
