...
def compute_angle_from_btrn(btrn_scores):
    """ 
        This method takes an array of shape [batch_size, seq_len, 2*nb_classes, 2] which represents the Biternion representation of angles
        and converts it into an array of angle representation in radians of shape [batch_size,  2*nb_classes]
    """
    #result_angles = np.zeros((btrn_scores.shape[0], btrn_scores.shape[1], btrn_scores.shape[2]))
    result_angles = np.arctan2(btrn_scores[:, :, :, 1], btrn_scores[:, :, :, 0]).reshape((btrn_scores.shape[0], btrn_scores.shape[1], btrn_scores.shape[2]))
